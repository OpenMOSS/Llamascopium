...
directory for JSON request files, forwards each request to localhost backend
HTTP, and writes JSON response files back to the shared directory.

This is intended for environments where the backend port is reachable on the
remote machine (for example http://127.0.0.1:3000) but cannot be exposed through
the notebook/proxy gateway.
"""
...
def forward_to_backend(payload: dict[str, Any], backend_url: str, timeout: float) -> dict[str, Any]:
    ...
    headers = {
        ...
        if str(k).lower() not in {"host", "content-length", "connection", "accept-encoding"}
    }
...
def process_one(request_path: Path, dirs: dict[str, Path], backend_url: str, timeout: float, archive: bool) -> None:
...
def main() -> int:
    ...
    print(f"[start] shared-file RPC worker", flush=True)
    print(f"[queue] {queue_dir}", flush=True)
    print(f"[backend] {args.backend_url}", flush=True)

    while not STOP:
        request_files = sorted(dirs["requests"].glob("*.json"), key=lambda p: p.stat().st_mtime)
