...
coefficient, and use an auxiliary loss to further improve dictionary utilization.

For the JumpReLU threshold, we use a separate learning rate for the learnable threshold parameters. 
We find that unconstrained threshold optimization can cause both the JumpReLU thresholds and encoder 
norms to grow excessively. To prevent this, we freeze the JumpReLU thresholds once the $L_0$ approaches the target $L_0$. 
We additionally apply global gradient clipping at $1.0$ to stabilize 
threshold optimization and set the $L_p$ coefficient to $0.003$.
...
\subsection{Gemma 3 JumpReLU Lorsa Hyperparameters}
\label{app:lorsa-hyperparameters}

\begin{table*}[t]
    \centering
    \caption{
        Training hyperparameters for Gemma 3 JumpReLU Lorsa modules.
        Each model family uses separate small- and big-$L_0$ runs. Dashes are
        placeholders for the final released hyperparameters.
    }
    \label{tab:gemma_lorsa_hyperparameters}
    ...
    \begin{tabular}{llrrrrrrrrrr}
        \toprule
        Model & Run & Width & Target $L_0$ & Learning rate & Token batch size
        & $\lambda$ & $\lambda_{\mathrm{aux}}$ & $\lambda_{\mathrm{aux}}$ warmup
        & Threshold LR factor & Threshold-freeze $L_0$ & Training tokens \\
        \midrule
        \multirow{2}{*}{Gemma 3 270M}
        & Small & -- & -- & -- & -- & -- & -- & -- & -- & -- & -- \\
        & Big   & -- & -- & -- & -- & -- & -- & -- & -- & -- & -- \\
        \midrule
